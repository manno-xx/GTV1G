...
public class TestMovement : MonoBehaviour
{
	...
	{
		// Old input system
		// _inputMovement = Input.GetAxis("Vertical") * _movementSpeed;
		// _inputTurning = Input.GetAxis("Horizontal") * _turningSpeed;
	...
	/// <summary>
	/// Move the Rigidbody.
	/// Is similar to _Steering Behaviors_ as described on https://www.red3d.com/cwr/steer/gdc99/
	/// </summary>
	private void Move()
	{
		//Get our target velocity, which is a sum of the player input and the current vertical velocity (due to falling or jumping)
		Vector3 desiredVelocity = transform.forward * _inputMovement + Vector3.up * _body.linearVelocity.y;
		//Calculate difference between our target velocity and our current velocity
		Vector3 steeringForce = desiredVelocity - _body.linearVelocity;
		...
		_body.AddForce(steeringForce, ForceMode.VelocityChange);
	}
}